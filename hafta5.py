...
import numpy as np
# Üye sayısını belirleyelim
n_members = 200

# Rastgele üye ID'leri ve üyelik paketleri oluşturalım
np.random.seed(42)
member_ids = np.arange(1, n_members + 1)
membership_packages = np.random.choice(['Standart', 'Premium'], size=n_members)

# Memnuniyet puanları, katılım sıklığı ve yaş gruplarını rastgele oluşturalım
satisfaction_scores = np.random.randint(1, 6, size=n_members)
attendance_frequency = np.random.randint(1, 31, size=n_members)  # Aylık ziyaret sayısı 1 ile 30 arasında
age_groups = np.random.choice(['18-25', '26-35', '36-45', '46-55', '56+'], size=n_members)

# Veri setini oluşturalım
gym_membership_df = pd.DataFrame({
    'Uye_ID': member_ids,
    'Uyelik_Paketi': membership_packages,
    'Memnuniyet_Puani': satisfaction_scores,
    'Katilim_Sikligi': attendance_frequency,
    'Yas_Grubu': age_groups
})

# Veri setini kaydedelim
output_file_path_gym = 'spor_salonu_uyelik_veriseti.xlsx'
gym_membership_df.to_excel(output_file_path_gym, index=False)

output_file_path_gym
